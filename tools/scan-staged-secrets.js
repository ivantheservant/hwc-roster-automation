...
const WORD_HEADS = [
  ...
  '施加', '范圍', '藍色', '黃色', '范例', '范疇',
  // 第四十七輪批次：`程式`／`程序` 喺度而 `程度` 唔喺——
  // 同一類詞漏一個，就會有一句「危險程度與……」被當成人名。
  '程度',
];
const WORD_TAILS = [
  ...
  // 第四十七輪批次：`張` 做量詞（一張表／嗰張表／成張表）。
  // 「張」係真姓氏，所以唔可以一律放行——但前面係數詞或者指示詞嗰陣，
  // 佢一定係量詞。
  '一張', '嗰張', '呢張', '這張', '那張', '每張', '成張', '幾張', '整張', '該張',
  // `游`／`許` 做詞尾。
  '上游', '下游', '容許', '也許', '或許', '少許', '允許', '不許', '唔許',
];
...
function scanChineseNames(lines) {
  ...
  lines.forEach(function (l) {
    ...
    while ((m = pattern.exec(l.text)) !== null) {
      ...
      // 第四十七輪批次：`連` 做副詞（「連 X 都 Y」／「連 X 一齊 Y」／
      // 「連 X 都改埋」）。呢個結構嘅標記唔喺前面，而喺**後面**——
      // 所以要向後望，唔可以靠 WORD_HEADS 逐個詞去砌。
      //
      // ⚠️ 逐個詞去砌（`連補`／`連司`／`連名`……）會令呢張表變成一張
      // 無止境嘅特例清單，而每加一個特例都會削弱佢本來要捉嗰樣嘢。
      if (name.charAt(0) === '連') {
        const after = l.text.slice(m.index + name.length, m.index + name.length + 8);
        if (/[都埋]|一齊/.test(after)) continue;
      }

      // 第四十七輪批次：`N 張表` —— 數字後面嗰個 `張` 一定係量詞。
      if (name.charAt(0) === '張' && /[0-9０-９]/.test(prev)) continue;

 ...
 */
/**
 * 第四十四輪批次 D 組：**分開「產生唔到」同「未產生過」。**
 *
 * ═════════════════════════════════════════════════════════════════════
 * ⚠️ 這一個分辨是整組 D 的關鍵
 * ═════════════════════════════════════════════════════════════════════
 *
 * `listPaperPackFiles_()` 把兩種完全不同的情況都放進同一個 `missing`：
 *
 *   一、`NameMapping 查不到這個編號。`
 *       這一位**永遠**都不會有 PDF。再產生一百次都一樣。
 *       要幹事自己去補 `NameMapping`。
 *
 *   二、`找不到「⋯⋯.pdf」。`
 *       這一位有名有姓，只是**那一份 PDF 還未產生過**。
 *       系統自己補得返——而且本來就有一個做開這件事的批次。
 *
 * Ivan 撞到的是第二種：他撳「寄到自己信箱」，系統回一句
 * 「一份個人 PDF 都找不到。」然後叫他去撳另一粒掣。
 * 系統明明知道欠什麼、明明有工具補得返，卻叫人手動去做一次。
 *
 * @param {Object[]} missing `listPaperPackFiles_()` 的 `missing`
 * @returns {{generatable: Object[], unfixable: Object[]}}
 */
function splitPaperPackMissing_(missing) {
  const generatable = [];
  const unfixable = [];
  (missing || []).forEach(function (m) {
    // 有名 ＝ `NameMapping` 查得到 ＝ 產生得返。
    if (m && String(m.nameTC || '').trim()) generatable.push(m);
    else unfixable.push(m);
  });
  return { generatable: generatable, unfixable: unfixable };
}

function apiEmailPaperPackToSelf(quarterId, personIds, extraEmails) {
  assertWebAppRequestAllowed_();
  // ⚠️ 第四十四輪批次 D 組：這一條路**由這一輪開始會寫入 Drive**
  //（欠幾份就自己補產生），所以一定要拿鎖。以前它純讀檔＋寄信，
  // 不拿鎖是對的；加了產生之後不拿，就會同「產生並取得連結」
  // 撞在一起，同一份 PDF 被兩邊同時寫。
  return withMutationLock_('寄紙本到自己信箱', function () {
    return apiEmailPaperPackToSelf_locked_(quarterId, personIds, extraEmails);
  });
}

function apiEmailPaperPackToSelf_locked_(quarterId, personIds, extraEmails) {
  const ids = (personIds || []).map(function (x) { return String(x || '').trim(); })
    .filter(function (x) { return x !== ''; });
  if (ids.length === 0) {
    throw new Error(buildThreePartMessage_(
  ...
  // ── 欠幾份 ⇒ 自己補產生，唔好叫幹事去撳另一粒掣 ─────────────
  //
  // ⚠️ 產生**唔可以**保證一次做得完：Apps Script 有六分鐘上限，
  // 而 `generatePersonalPdfBatchForPeople_()` 本來就有斷點續做。
  // 所以做唔完嘅時候，**一封都唔寄**，改為報進度、叫佢再撳一次。
  //
  // 點解唔寄住嗰幾份先：幹事收到一封夾住 30 份嘅信，唔會逐份數。
  // 佢會印晒、派晒，然後有幾位企喺度冇紙。一封唔完整嘅信
  // 比一句「仲差 12 份，再撳一次」差得多。
  let found = listPaperPackFiles_(quarterId, versionNo, ids);
  let split = splitPaperPackMissing_(found.missing);
  let autoBatch = null;

  if (split.generatable.length > 0) {
    const needIds = split.generatable.map(function (m) { return m.personId; });
    autoBatch = generatePersonalPdfBatchForPeople_(quarterId, versionNo, needIds);
    found = listPaperPackFiles_(quarterId, versionNo, ids);
    split = splitPaperPackMissing_(found.missing);

    writeAuditLog_({
      action: 'PAPER_PACK_AUTO_GENERATED',
      targetSheet: buildRosterSheetName_(quarterId, versionNo),
      targetCell: '',
      oldValue: '',
      newValue: '寄之前補產生 ' + needIds.length + ' 份，完成 '
    ...
    if (!autoBatch.done || split.generatable.length > 0) {
      ...
      return {
        ...
        missing: split.unfixable,
        stillGeneratingCount: stillShort,
        extraCount: extra.length,
        message: '這一次幫你補產生了 ' + autoBatch.doneCount + ' 份個人 PDF，'
          + '還差 ' + stillShort + ' 份未產生好，所以一封都未寄出。'
          + '再撳一次「寄給我自己」，系統會由停低的位置接住做；'
          + '做齊之後就會立刻寄出。'
      };
    }
  }
  ...
  return {
    ...
    // 但畫面要講得出「寄咗去你自己個信箱」。
    extraCount: extra.length,
    // 第四十四輪批次 D 組：補產生過就要講——幹事撳嗰粒掣嗰陣以為
    // 只係寄信，而系統其實幫佢做埋產生。唔講嘅話，佢下次仲會以為
    // 自己要先撳「產生並取得連結」。
    autoGeneratedCount: autoBatch ? autoBatch.generatedCount : 0,
    message: '已經寄出 ' + sentCount + ' 封到你自己的信箱'
      + (extra.length > 0 ? ('（同你另外輸入的 ' + extra.length + ' 個地址）') : '')
      + '，共 ' + found.files.length + ' 份。'
      + (autoBatch && autoBatch.generatedCount > 0
        ? ('其中 ' + autoBatch.generatedCount + ' 份是系統在寄之前替你補產生的。')
        : '')

 ...
 *
 * ─────────────────────────────────────────────────────────────────────
 * ⚠️⚠️ 第三十二輪批次階段 C′：`weeksCounted` 同量度介面刻意唔一致
 * ─────────────────────────────────────────────────────────────────────
 *
 * **這個函式是 greedy pass 內部的「節流參數」，不是量度。**
 *
 * 量度介面（`Verify.gs` 品質統計、`RuleReview.gs` 規則審閱表）一律用
 * 相鄰對數 `週數 − 1`（＝ `adjacentPairCount_()`）——兩者必須一致，
 * 而且已經一致。
 *
 * 引擎這裡繼續用 `weeksCounted`（＝ 13 而不是 12）是**刻意保留**的：
 *
 *   ・第三十二輪實測 15 個 seed，改成 `weeksCounted − 1` 會令
 *     **15 張職事表全部改變**，並且連鎖影響主席／報告是否同一人
 *     （seed 1：30.8% → 38.5%）——而目標指標**零改善**
 *     （新舊都是 4 對／33.3%）。
 *   ・現場實測：所有已生成版本之中，報告連續 **25.0% 出現 20 次、
 *     33.3% 出現 3 次**，平均 3.13 對。目標 `0.27 × 12 = 3.24` 對。
 *     **引擎現場已經命中目標。**
 *   ・12 對之下，3.24 對只能實現為 3 對或 4 對。25% 與 33% 的分別
 *     就是一對——那是量化雜訊，任何分母微調都不可能排出 27%。
 *
 * 換言之：改它要付「每一季排表結果全部改變」的代價，換回來的只是
 * 一個寫法一致，而它想改善的東西本來就沒有壞。
 *
 * ⚠️ **不要「順手」把這裡改成 `adjacentPairCount_()`。**
 * `tests/announce_pair_denominator_impact.test.js` 會擋住，
 * 而那條測試的檔頭寫住完整的實測數字。
 *

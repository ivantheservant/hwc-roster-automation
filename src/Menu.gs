  ...
  ui.createMenu('職事表系統')
    ...
    .addSubMenu(
      ui.createMenu('測試工具')
        ...
        // 第三十三輪批次階段 D3：兩項並排、字頭一模一樣，實測時 Ivan 撳錯咗
        //（想跑第一段，撳咗接續）。而家字頭同圖示都唔同：
        //   ⚠️⚠️ 全季流程演練（第一段：由頭開始）
        //   ▶️ 全季流程演練（接續上一段）
        // 幹事培訓時呢兩粒掣會再被撳錯一次，而家改成本最低。
        .addItem('⚠️⚠️ 全季流程演練（第一段：由頭開始）', 'runSeasonRehearsal_')
        // 第三十二輪批次階段 B：一次執行做唔晒 58 份個人 PDF（實測要
        // 分 3 次、共 552 秒，而單次執行上限 6 分鐘）。呢一項就係
        // 把同一次演練行完嘅路，包括步驟 4、5——嗰兩步至今
        // 從來冇喺演練入面行過。
        .addItem('▶️ 全季流程演練（接續上一段）', 'runSeasonRehearsalResume_')
    )
    .addSubMenu(

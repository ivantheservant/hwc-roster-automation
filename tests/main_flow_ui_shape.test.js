...
console.log('\n=== A1：掣上面直接寫住季度，唔使佢揀 ===');
{
  ...
  // ⚠️ 第四十二輪批次 C 組之後，呢個閘多咗一個觸發條件
  //（`goLiveNote`——真正上線嗰一季亦都要先問一次）。
  // 斷言要守嘅係「有警告就一定要先彈確認」，唔係嗰句 `if` 嘅字面。
  check('★★★★★ 已經開始／已經過去／未到生成日期要**先彈警告先做**，唔可以直接做',
    /if \(t\.warn \|\| goLiveNote\) \{[\s\S]{0,600}openConfirm\(/.test(flow), '');
  check('★★★★★ 而且真正要上線嗰一季亦都要先問一次'
    + '——Ivan 留咗一季做真正上線，撳落去就會用掉佢',
    /t\.isGoLiveQuarter/.test(flow) && /這一季是實際要上線的季度/.test(flow), '');

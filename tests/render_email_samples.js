...
lines.push('');
lines.push('### 3. 一季一條固定連結：更新 4 個既有範本');
lines.push('');
lines.push('第十一輪批次新增「一季一條固定連結」（詳見 `docs/系統範圍稽核.md`），');
lines.push('步驟 4／5 嘅信入面加咗一句公開職事表連結。**「補齊 Email 範本」呢個');
lines.push('工具唔會幫你做呢一步**——嗰個工具只新增缺少嘅範本，唔會覆寫已經');
lines.push('存在嘅範本內容，而以下 4 個範本喺你嘅工作表已經存在。要令新內容');
lines.push('生效，需要自己去 `EmailTemplates` 逐個手動更新 `BodyHtml`／');
lines.push('`BodyPlain`／`Placeholders` 三欄（`Subject`／`Stage`／`Lang`／');
lines.push('`AttachType` 冇變，唔使改）。');
lines.push('');
lines.push('⚠️ **順序建議**：先執行一次「準備工作 ▸ ⚠️ 發佈公開職事表」，');
lines.push('`{PublicRosterUrl}` 先有真正嘅連結可以代入；未發佈過都唔會出錯，');
lines.push('淨係嗰句連結會留空（範本入面嗰句字照樣出現，但連結係空嘅），');
lines.push('唔會令寄信整個失敗。');
lines.push('');
const publicLinkTemplateIds = ['TPL_OFFICIAL_TC', 'TPL_OFFICIAL_LIST_TC', 'TPL_RESEND_TC', 'TPL_RESEND_LIST_TC'];
publicLinkTemplateIds.forEach(function (id) {
  const found = samples.filter(function (s) { return s.template.templateId === id; })[0];
  const t = found.template;
  lines.push('#### `' + id + '`');
  lines.push('');
  lines.push('`Placeholders`（整欄取代成呢句）：');
  lines.push('');
  lines.push('```');
  lines.push(t.placeholders);
  lines.push('```');
  lines.push('');
  lines.push('`BodyHtml`：');
  lines.push('');
  lines.push('```html');
  lines.push(t.bodyHtml);
  lines.push('```');
  lines.push('');
  lines.push('`BodyPlain`：');
  lines.push('');
  lines.push('```');
  lines.push(t.bodyPlain);
  lines.push('```');
  lines.push('');
});
